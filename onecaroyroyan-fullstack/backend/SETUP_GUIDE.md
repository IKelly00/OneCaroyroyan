...
| Dashboards (all 5 roles) | *(aggregation queries across the above)* | Live stat cards, tables, and charts |

### Changed in the frontend (`onecaroyroyan/src`)
- **`src/lib/api.js`** *(new)* — fetch client for every endpoint above.
...
FinReports, CaptainReports, and Settings** *list/detail pages* (as opposed
...
You should see:
```
OneCaroyroyan API listening on http://localhost:5000
```
